...
def dispatcher_loop():
	...
	while True:
		...
		with thread_lock:
			if len(client_rrQueue) == 0:
				continue
			cid = client_rrQueue[rr_index % len(client_rrQueue)]
			rr_index += 1

			if cid in client_frame:
				client_data = client_frame[cid]
				# มีเฟรมใหม่รอประมวลผลหรือไม่?
				if client_data['seen'] < client_data['seq']:
					try:
						job_queue.put_nowait((cid, client_data['payload']))
					except queue.Full:
						# คิวเต็ม → ข้ามรอบ เพื่อรักษา latency ของทั้งระบบ
						pass

		# -------- (2) ส่งงานเข้าหมวด worker (จำกัดไม่เกิน MAX_WORKERS ต่อรอบ) --------
		for _ in (range(MAX_WORKERS)):
			try:
				_cid, _payload = job_queue.get_nowait()
				thread_pool.submit(yolo_frame_process, _cid, _payload)  # งานหนักรันใน OS threads
			except queue.Empty:
				break
		...
		while not result_queue.empty():
			try:
				_cid, event_name, data = result_queue.get_nowait()

				# อัปเดต seen ให้ทัน seq ปัจจุบัน (ลดการประมวลผลเฟรมซ้ำ)
				with thread_lock:
					if _cid in client_frame:
						client_frame[_cid]['seen'] = client_frame[_cid]['seq']

				# emit นอก Lock เสมอ เพื่อลดเวลาถือล็อก
				socketio.emit(event_name, data, room=_cid)

			except queue.Empty:
				pass
			except Exception as e:
				# จับ error ช่วง emit เพื่อกันลูปตาย
...
def on_connect():
	...
	with thread_lock:
		if cid not in client_rrQueue:
			client_rrQueue.append(cid)
	socketio.send(f"Client \"{cid}\" connected.", room=cid)
...
def on_disconnect():
	...
	with thread_lock:
		if cid in client_frame:
			del client_frame[cid]
		if cid in client_rrQueue:
			client_rrQueue.remove(cid)
...
def on_video_frame(data):
	...
	* ไม่ทำงานหนักใน def นี้ (ลด latency ของ event loop)
	...
	cid = request.args.get('cid', sid)
	with thread_lock:
		if cid not in client_frame:
			client_frame[cid] = {
				'payload': data,  # {'task','imgB64','params':{...}}
				'seq': 0,
				'seen': -1
			}
		else:
			client_frame[cid]['payload'] = data
			client_frame[cid]['seq'] += 1
		if cid not in client_rrQueue:
			client_rrQueue.append(cid)
	# socketio.send(f"Server received frame[{client_frame[cid]['seq']}]", room=cid)
